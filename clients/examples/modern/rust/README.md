...
crate (1.x) plus the [`connect/rust`](../../../connect/rust) helper crate (`efdi_connect`).
...
read-only. The `efdi_connect` helper does it the working way: it serializes the whole block
...
- `efdi_connect` also exposes `session_blocking()` (via zenoh's `Wait` adapter) for callers
