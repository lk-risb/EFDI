...
//
// The mesh-side mTLS (EFDI_CERT/EFDI_KEY/EFDI_CA, the "one TLS block" gotcha) lives in the POD's
...
//
//     EFDI_ROUTER       tls/127.0.0.1:7447   pod's NATIVE Zenoh endpoint. Used here only to
//                                            derive the plugin host when EFDI_WS is unset.
//     EFDI_WS           ws/127.0.0.1:10000   remote-api plugin WebSocket locator (preferred).
//                                            If unset, defaults to ws/<EFDI_ROUTER host>:10000.
//     PARTNER_NAMESPACE    release/<you>        your owned prefix (publish under this)
//
//     EFDI_CERT / EFDI_KEY / EFDI_CA / EFDI_VERIFY_NAME — present for parity with the other
 ...
 *
 * Prefers EFDI_WS (e.g. "ws/127.0.0.1:10000"). If unset, derives it from EFDI_ROUTER's host on
 * the default plugin port 10000 — e.g. EFDI_ROUTER="tls/127.0.0.1:7447" -> "ws/127.0.0.1:10000".
...
export function wsLocator(): string {
  const raw = process.env.EFDI_WS ?? deriveWsFromRouter(env("EFDI_ROUTER"));
...
export function namespace(): string {
  return env("PARTNER_NAMESPACE").replace(/\/+$/, "");
}
