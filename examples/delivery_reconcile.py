...
the pub/sub delivery-reconciliation pattern; this is a
...
stops landing.
  ...
  Tier 3:           must-land messages → Query/Reply admission (not shown;
                    see the delivery-reconciliation pattern).
...
def self_canary(session: zenoh.Session, prefix: str, intent_ledger: set[str]) -> set[str]:
    ...
    TTL-expired — for a definitive denied-vs-allowed test, use a Query/Reply
    admission check (Tier 3) instead.
    """
