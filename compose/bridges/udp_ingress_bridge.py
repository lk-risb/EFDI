...
Every datagram is preserved in a RawEnvelope on ``raw/udp/ingress``. When the
complete datagram is unambiguously framed as ASTERIX, each frame is also
published unchanged to:
...
Unknown protocols are never guessed or discarded; a protocol-specific
translator can be attached to the generic raw topic after its wire contract is
identified. This bridge intentionally does not interpret any ASTERIX UAP.
"""
...
def run(args) -> None:
    ...
    print(
        "Generic UDP ingress on {}:{}; ASTERIX auto-dispatch categories={}".format(
            args.bind,
            args.port,
    ...
    try:
        ...
        while True:
            ...
            if not source_allowed(source_ip, allowed_sources):
                if args.verbose:
                    print("UDP rejected source", source_ip, flush=True)
                continue
            sequence += 1
            protocol = "unknown"
            profile = ""
            try:
                ...
                if args.verbose:
                    print(
                        "UDP unclassified {} bytes from {}:{} ({})".format(
                            len(datagram), source_ip, address[1], exc
                        ),
                        flush=True,
                    )
            else:
                protocol = "asterix"
                profile = ",".join(
                    "cat{}".format(category) for category, _frame in frames
                )
            ...
            envelope = RawEnvelope(
                ...
                protocol=protocol,
                transport="udp",
                source_address="{}:{}".format(source_ip, address[1]),
                sequence=sequence,
                payload=datagram,
            ...
            raw_publisher.put(
                ...
                encoding=zenoh.Encoding.APPLICATION_PROTOBUF.with_schema(
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generic UDP capture with safe ASTERIX auto-dispatch"
    )
    parser.add_argument(
        "--bind",
        default=os.environ.get(
            "UDP_INGRESS_BIND", os.environ.get("ASTERIX_BIND", "0.0.0.0")
        ),
    )
    parser.add_argument(
        "--port",
        ...
        default=int(
            os.environ.get(
                "UDP_INGRESS_PORT", os.environ.get("ASTERIX_PORT", "50000")
            )
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--multicast-group",
        default=os.environ.get(
            "UDP_INGRESS_MULTICAST_GROUP",
            os.environ.get("ASTERIX_MULTICAST_GROUP", ""),
        ),
    )
    parser.add_argument(
        "--multicast-interface",
        default=os.environ.get(
            "UDP_INGRESS_MULTICAST_INTERFACE",
            os.environ.get("ASTERIX_MULTICAST_INTERFACE", "0.0.0.0"),
        ),
    )
    parser.add_argument(
        "--allow-source",
        ...
        default=[
            os.environ.get(
                ...
                os.environ.get("ASTERIX_ALLOW_SOURCE", ""),
            )
        ],
