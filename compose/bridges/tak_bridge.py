...
When enabled explicitly, TAK point map markers (`b-m-p-*`) are published as
unknown C2 units so the standard TAK and SitaWare output layers distribute
them too.  This preserves the marker's CoT type for TAK while keeping it out
of the user-position path.
...
def _is_point_marker(track: dict) -> bool:
    """Return true only for ordinary CoT point map markers, not drawings/chat."""
    return str(track.get("cot_type", "")).lower().startswith("b-m-p-")


def _should_publish(track: dict, tak_users_only: bool, include_markers: bool = False) -> bool:
    """Reject our own echoes and optionally admit only user SA plus point markers."""
    if str(track.get("uid", "")).upper().startswith("EFDI-"):
        return False
    if not tak_users_only:
        return True
    return track.get("tak_user") is True or (include_markers and _is_point_marker(track))


def _topic(track: dict) -> str:
    if _is_point_marker(track):
        # A C2 map marker is represented as an unknown land unit for SitaWare
        # NVG compatibility. cot_layer retains the original b-m-p-* type for
        # TAK, rather than recategorising it as a vehicle or aircraft.
        return "{}/land/c2/cot/unknown/unit/tracks/v1".format(TOPIC_ROOT)
    ...
    try:
        while True:
            ...
            for xml_str in messages:
                ...
                if track is None or not _should_publish(
                    track, tak_users_only, include_markers
                ):
                    continue
                track["_src"] = source
