...
const FIELD_GROUPS: { title: string; description: string; fields: { key: string; label: string; secret?: boolean; placeholder?: string }[] }[] = [
  { title: 'Router and namespace', description: 'Native-process endpoint and namespace defaults. Use Zenoh Config for live router listener changes.', fields: [
    ...
    { key: 'ZENOH_LOG', label: 'Router log level (restart required)', placeholder: 'info' },
  ] },
  { title: 'Fabric presets', description: 'One-click "Switch fabric" options shown on the Zenoh Config page. Changing these changes what appears as a pill there, not the currently active connection.', fields: [
    { key: 'EFDI_LOCAL_FABRIC_LABEL', label: 'Local fabric — preset label', placeholder: 'Local fabric' },
    { key: 'EFDI_LOCAL_FABRIC_ENDPOINTS', label: 'Local fabric — endpoints (JSON array)', placeholder: '["tls/zenoh1.efdi.ltu:7447"]' },
    { key: 'EFDI_LOCAL_FABRIC_PROFILE', label: 'Local fabric — TLS profile', placeholder: 'ltu-local' },
    { key: 'EFDI_BACKBONE_FABRIC_LABEL', label: 'Backbone fabric — preset label', placeholder: 'Backbone' },
    { key: 'EFDI_BACKBONE_FABRIC_ENDPOINTS', label: 'Backbone fabric — endpoints (JSON array)', placeholder: '["tls/backbone.efdi.ltu:7447"]' },
  ...
  { title: 'SitaWare HQ', description: 'REST import (sitaware_bridge) and NVG 2.0.2 export feed (sitaware_layer). Passwords are write-only.', fields: [
    ...
    { key: 'SITAWARE_HQ_NVG_ENABLE', label: 'NVG feed enabled — Zenoh → HQ polls (nvg_layer) (1/0)', placeholder: '1' },
