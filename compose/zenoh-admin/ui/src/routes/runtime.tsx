                  ...
                  >
                    <div className="flex items-center gap-3">
                      <input
                        ...
                        className="h-4 w-4 rounded-lg border-zinc-400 bg-transparent accent-accent-fill focus:ring-accent-ring disabled:opacity-30 dark:border-zinc-600"
                      />
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-1.5">
                          ...
                          {item.kind && (
                            <span className={`shrink-0 rounded-lg px-1.5 py-0.5 text-[9px] font-medium uppercase tracking-wide ${KIND_STYLE[item.kind] ?? KIND_STYLE.infrastructure}`}>
                              {item.kind}
                            </span>
                          )}
                        </div>
                      ...
                      <button title={blockedReason ? `Cannot start — ${blockedReason}` : 'Start'} disabled={!canWrite || !!action || state?.running || !!blockedReason} onClick={() => serviceAction(item.name, 'start')} className="rounded-lg p-1.5 text-zinc-500 transition hover:bg-emerald-500/10 hover:text-emerald-500 disabled:opacity-30"><Play size={13} /></button>
                      <button title={item.kind === 'infrastructure' ? 'Infrastructure cannot be stopped from here' : 'Stop'} disabled={!canWrite || !!action || !state?.running || item.kind === 'infrastructure'} onClick={() => serviceAction(item.name, 'stop')} className="rounded-lg p-1.5 text-zinc-500 transition hover:bg-red-500/10 hover:text-red-500 disabled:opacity-30"><Square size={12} /></button>
                      <button title={blockedReason ? `Cannot restart — ${blockedReason}` : 'Restart'} disabled={!canWrite || !!action || item.name === 'admin-control' || !!blockedReason} onClick={() => serviceAction(item.name, 'restart')} className="rounded-lg p-1.5 text-zinc-500 transition hover:bg-accent-ring/10 hover:text-accent-ring disabled:opacity-30"><RefreshCw size={13} className={action === 'restart' ? 'animate-spin' : ''} /></button>
                      <button title="Show logs" onClick={() => showLogs(item.name)} className="rounded-lg p-1.5 text-zinc-500 transition hover:bg-zinc-200 dark:hover:bg-white/10"><FileText size={13} /></button>
                    </div>
                    {openLogs === item.name && <pre className="relative z-10 mt-2 max-h-40 overflow-auto rounded-lg bg-zinc-950 p-2 font-mono text-[10px] leading-4 text-zinc-300">{logLines.join('\n') || 'No log output yet.'}</pre>}
                  </div>
                )
              })}
            </div>
