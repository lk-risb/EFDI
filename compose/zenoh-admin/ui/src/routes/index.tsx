...
function StatCard({ label, value }: { label: string; value: string }) {
  return (
    <div className="hud-frame rounded-md border border-zinc-200 dark:border-white/10 hud-glass p-5 hud-card">
      ...
      <p title={value} className="mt-2 truncate text-xl font-semibold tracking-tight text-zinc-900 dark:text-zinc-100">{value}</p>
    </div>
  )
}
        ...
        <div className="mt-3 space-y-2 max-h-80 overflow-auto">
          {peers.map(p => (
            <div key={p.zid} className="flex items-center gap-2 text-sm">
              ...
              <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-lg border text-zinc-600 dark:text-zinc-400 bg-zinc-200 dark:bg-zinc-400/10 border-zinc-300 dark:border-zinc-700 shrink-0">
            ...
            return (
              <div key={group}>
                <button onClick={() => toggle(group)}
                  ...
                  <span className={cn('text-xs font-semibold px-1.5 py-0.5 rounded-lg border shrink-0', colorClass)}>{group}</span>
...
function SystemStatCard({ icon: Icon, label, value, state = 'ok' }: { icon: LucideIcon; label: string; value: string; state?: 'ok' | 'warn' | 'critical' }) {
  ...
  return (
    <div className="hud-frame rounded-md border border-zinc-200 dark:border-white/10 hud-glass p-5 hud-card">
      ...
      <p title={value} className={cn('truncate text-xl font-semibold tracking-tight', textClass)}>{value}</p>
    </div>
  )
}
...
function ServiceCard({ service }: { service: ServiceState }) {
  ...
  return (
    <div className="hud-frame rounded-md border border-zinc-200 dark:border-white/10 hud-glass p-5 hud-card">
      ...
      <p title={service.name} className="mb-2 truncate text-base font-semibold text-zinc-900 dark:text-zinc-100">{service.name}</p>
  ...
  return (
    <Layout>
      <div className="p-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-6 gap-4 mb-6 hud-enter">
          {system ? (
            ...
            Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="hud-frame rounded-md border border-zinc-200 dark:border-white/10 hud-glass p-5 hud-card">
                <HudCorners />
                <div className="flex items-start justify-between mb-3">
        ...
        {services.length > 0 && (
          <>
            <h2 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100 mb-3">Services</h2>
        ...
        {status && (
          <>
            <h2 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100 mb-3">Zenoh</h2>
