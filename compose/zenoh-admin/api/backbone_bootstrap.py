...
async def upload_backbone_identity(
    ...
    # generic_json/geojson/camera_sites/socbx/palantir/tacvox/tytan/usareur/
    # skylord/kyber/fpv/ita_efdi (the normalizers backbone-bridge's raw
    # output feeds), and asterix (cat34/cat48's --zenoh-raw mode turns on
    # once this identity file exists — see start.sh's
    ...
    for service in ("backbone-bridge", "backbone_layer", "generic_json", "geojson", "camera_sites", "socbx", "palantir", "tacvox", "tytan", "usareur", "skylord", "kyber", "fpv", "ita_efdi", "asterix"):
        try:
