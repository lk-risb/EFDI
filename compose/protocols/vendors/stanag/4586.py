#!/usr/bin/env python3
"""Deployment-profile STANAG 4586 UAS interface adapter → Zenoh.
...
The public NATO material does not define the edition- and VSM-specific binary
wire profile. The historical EFDI layout below is therefore disabled unless
``STANAG4586_PROFILE=legacy_ed3_approx`` is explicitly selected after checking
it against the target VSM ICD. It must not be treated as generic STANAG 4586.
...
Run:
  venv/bin/python3 protocols/vendors/stanag/4586.py --host 192.168.1.50 --port 4586
"""
...
Stanag4586Track = import_module("protocols.vendors.stanag.4586_pb2").Stanag4586Track
...
SUPPORTED_PROFILE = "legacy_ed3_approx"
...
def _require_profile() -> None:
    ...
    if profile != SUPPORTED_PROFILE:
        raise SystemExit(
            ...
            "against the deployed VSM ICD.".format(SUPPORTED_PROFILE)
        )
...
def _recv_message(sock: socket.socket) -> tuple[int, int, bytes]:
    ...
    if total_size > MAX_FRAME_BYTES:
        raise ValueError(f"STANAG 4586 message exceeds {MAX_FRAME_BYTES} bytes")
...
def run(args):
    ...
    print("  Profile: {} (deployment-validated opt-in required)".format(SUPPORTED_PROFILE), flush=True)
    ...
    def on_sample(sample):
        try:
            ...
            while len(buffer) >= HEADER_SIZE:
                ...
                if total_size < HEADER_SIZE or total_size > MAX_FRAME_BYTES:
                    del buffer[0]
                    continue
                if len(buffer) < total_size:
                    break
                msg_type, _instance, body = _recv_message_from_bytes(bytes(buffer[:total_size]))
...
def _recv_message_from_bytes(frame: bytes) -> tuple[int, int, bytes]:
    ...
    return msg_type, instance, frame[HEADER_SIZE:]
