...
def publish_native(session, topic: str, payload: bytes, protocol: str, zenoh,
    ...
    record_out("egress-raw", len(data))
...
def publish_dual(
    ...
    try:
        from protocols.vendors.sapient.flex335 import publish_sapient
    ...
    json_payload = json.dumps(track, separators=(",", ":")).encode()
    session.put(
        view_key(key, "json"),
        json_payload,
        encoding=zenoh.Encoding.APPLICATION_JSON,
    )
    record_out("egress-json", len(json_payload))
    ...
    record_out("egress-proto", len(payload))
