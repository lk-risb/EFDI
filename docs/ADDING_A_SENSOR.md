# Adding a new sensor or protocol
...
and running (see [`INSTALL.md`](INSTALL.md)).

Read [`INTEGRATIONS.md`](INTEGRATIONS.md) first if you haven't — it explains
the fabric this walkthrough plugs into (the topic taxonomy, the four output
views, what's already wired). This doc is the concrete "now build one" steps;
that one is the reference for what already exists.

## 0. Decide: bridge, or protocol?
...
## 1. Do you need a new message schema, or does an existing one fit?
...
`NormalizedTrack` schema (`compose/protocols/random/normalized_track.proto`)
...
1. Add a new `.proto` file under `compose/protocols/random/` (or
   `compose/protocols/vendors/<name>/` for a licensed/branded standard),
...
from translation_common import make_config
...
def run() -> None:
    ...
    for raw in your_data_source():          # poll an API, read a socket, etc.
        ...
        if record:
            publish_dual(session, OUTPUT_TOPIC, record, NormalizedTrack, zenoh)
```

`publish_dual` does the rest: it publishes all four fabric views (`/sapient`,
`/json`, `/proto`, `/raw`) from that one call — see
[`INTEGRATIONS.md` "Egress topic views"](INTEGRATIONS.md#egress-topic-views-sapient-json-proto-raw)
for what each view is for. You never publish to TAK or SitaWare directly —
`cot_layer`/`nvg_layer` subscribe to every normalized topic on the fabric
automatically, so a correctly-published track appears in both without any
further code.

**Topic path.** Follow the taxonomy from `INTEGRATIONS.md`:
...
## 3. Register it with the launcher
...
## 5. New CoT symbol needed? (TAK output only)

If your sensor's affiliation/entity combination doesn't already map to a CoT
type, add it to `_TOPIC_COT` in `compose/layers/cot_layer.py`:
```python
...
Add a row to the relevant table in [`INTEGRATIONS.md`](INTEGRATIONS.md) (under
...
- [ ] A row added to `INTEGRATIONS.md`.
